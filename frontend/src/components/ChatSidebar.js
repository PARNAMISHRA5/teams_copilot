...
const ChatSidebar = ({
  ...
  const handleDeleteChat = async (chatId, e) => {
    ...
    if (window.confirm('Are you sure you want to delete this chat?')) {
      setDeletingChatId(chatId); // Set the chat ID being deleted
      try {
        await onDeleteChat(chatId); // Call the parent's delete function
      } finally {
        setDeletingChatId(null); // Clear the deleting state regardless of success/failure
      }
    }
  };
